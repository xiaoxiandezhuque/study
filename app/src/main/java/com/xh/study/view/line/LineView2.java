...
public class LineView2 extends View {
    ...
    private boolean isSelect;//开启滑动
    private boolean isClick;//开启点击
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isDownSelect) {
            mPaint.setColor(Color.parseColor("#3316A086"));
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_UP:
                    if (mOnSelectClickListener != null) {
                        mOnSelectClickListener.click((int) (event.getX() / spacing / mHowMuchData));
                    }
                    break;
            }
            return true;
    ...
    public void setSelectTime(int starTime, int endTime) {
        mOnSelectListener.select(starTime, endTime);
        isDownSelect = true;
        downX = (int) (mHowMuchData * starTime * spacing);
        moveX = (int) (mHowMuchData * endTime * spacing);
    }
