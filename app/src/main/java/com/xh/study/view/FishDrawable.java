...
public class FishDrawable extends Drawable {
    ...
    //控制鱼鳍的控制点比例
    private float controlFinsScale = 1f;
    private boolean isOpenFinsMotion = false;
    ...
    private void makeFins(Canvas canvas, PointF startPoint, float fishAngle, boolean isRight) {
        ...
        if (isOpenFinsMotion) {
            controlFinsScale = (float) (Math.abs(Math.sin(Math.toRadians(currentValue * 1.5))) / 2 + 0.5);
        } else {
            controlFinsScale = 1;
        }
        PointF controlPoint = calculatePoint(startPoint, FINS_LENGTH * 1.8f * controlFinsScale,
    ...
    public boolean isOpenFinsMotion() {
        return isOpenFinsMotion;
    }

    public void setOpenFinsMotion(boolean openFinsMotion) {
        isOpenFinsMotion = openFinsMotion;
    }
}
