...
//        paint.setXfermode(PorterDuffXfermode(PorterDuff.Mode.SRC_IN))// 设置转换模式（显示Scr与Dst交接的区域）
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
        ...
        roundedStyle =
            typedArray.getInt(R.styleable.RoundedImageView_roundedStyle, RoundedStyle.ARC.value)
    ...
    private fun initClipBitmap(w: Int, h: Int) {
        when (roundedStyle) {
            ...
            RoundedStyle.ARC_TOP.value -> onArcTop(w, h)
...
//    }
    ...
    private fun onArcTop(w: Int, h: Int) {
        if (radius == 0f) {
            clipBitmap = null
            return
        }
...
//        //右下角
//        val rbF = RectF(w.toFloat(), h.toFloat(), w.toFloat(), h.toFloat())
//        path.arcTo(rbF, 0f, 90f, false)
//        //左下角
//        val lbF = RectF(0f, h.toFloat(), 0f, h.toFloat())
//        path.(lbF, 90f, 90f, false)
        createClipBitmap(w, h, path)
    }
...
//        canvas?.drawBitmap(ImageUtils.drawable2Bitmap(drawable),rect!!,rect!!,clipPaint)
