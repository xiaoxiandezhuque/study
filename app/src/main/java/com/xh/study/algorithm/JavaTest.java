...
//    因为把21345 的最高位去掉就变成1345 ，便于我们采用递归的思路。
     ...
     */
    public static int test29(int n) {
        if (n <= 0) {
            return 0;
        }
    ...
    public static int test29_zishixian(int num) {
        if (num <= 0) {
            return 0;
        }
        String n = num + "";
        ...
        return huoqu1(nums, 0);
    }

    private static int huoqu1(int[] nums, int start) {
        ...
        int sum = 0;
        if (length >= 2) {
            sum = powerBase10(length - 2) * (length - 1) * nums[start];
            if (nums[start] > 1) {
                sum = sum + powerBase10(length - 1);
            } else {
                sum = sum + atoi(nums, start + 1) + 1;
            }
        } else {
            sum++;
        }
        if (length > 1) {
            sum += huoqu1(nums, ++start);
        }
        return sum;
    }
